...
rm(list = ls())
setwd("C://Users//andre//Desktop//Univ stuff//Year 2//Master Project//interface")
# Load data:
...
res.aov.level <-aov(logTime ~ Level, data = dat_ans)
...
leveneTest(logTime ~ Level, data = dat_ans)
...
# results: still significant; levels 3-4 become significantly different
res.kruskal.level <- kruskal.test(logTime ~ Level, data = dat_ans)
res.kruskal.level

dunnTest(logTime ~ Level, data=dat_ans)
```
...
res.kruskal.scen <- kruskal.test(logTime ~ Scenario, data = dat_ans_blocktwo)
res.kruskal.scen

dunnTest(logTime ~ Scenario, data=dat_ans_blocktwo)
```
...
acc.table.level <- table(dat_ans$Is.correct, dat_ans$Level)
...
# results: not significant
n_low_enjoy <- nrow(dat_ans[dat_ans$Enjoy.puzzle < 6,])
x_low_enjoy <- nrow(dat_ans[dat_ans$Enjoy.puzzle < 6 & dat_ans$Is.correct == "1",])
n_high_enjoy <- nrow(dat_ans[dat_ans$Enjoy.puzzle > 8,])
x_high_enjoy <- nrow(dat_ans[dat_ans$Enjoy.puzzle > 8 & dat_ans$Is.correct == "1",])
